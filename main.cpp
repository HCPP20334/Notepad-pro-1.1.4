	...
	if(bSave) {
		...
		if(GetOpenFileName(&ofn)) {
			if(!LoadFile(GetDlgItem(hwnd, IDC_MAIN_TEXT), szFileName)) {
				MessageBox(hwnd, "При загрузки файла прошзошла ошибка!!.", "Ошибка",MB_OK|MB_ICONEXCLAMATION);
				return FALSE;
			}
		}
	}
		...
		case WM_COMMAND:
			switch(LOWORD(wParam)) {
				...
				case CM_FILE_EXIT:
					...
					break;
				case CM_ABOUT:
					MessageBox (NULL, "Notepad pro 1.1.4 by Hackerman_20334" , "О продукте", 0);
					break;
				case CM_START:
					 WinExec("Lite.exe", 1);
						break;
				case CM_GIT:
					system("start /min /wait msedge github.com/yurokHackerman222");
					break;
				case CM_VER:
					MessageBox(NULL, "Версия программы 1.1.4","Версия ПО", 0);
					break;
				case CM_HELP:
					MessageBox(NULL, "Нажмите OK И Выберите в пути с приложением файл README.txt","Как пользоваться", 0);
				    DoFileOpenSave(hwnd, FALSE);
					break;
				case CM_NEW:
					MessageBox(NULL, "Версия программы 1.1.4","Проверка обновления",1);
					break;
			}
			break;
		case WM_CLOSE:
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,LPSTR lpCmdLine, int nCmdShow) {
	...
	wc.hCursor       = LoadCursor(NULL, IDI_ASTERISK);
	...
	hwnd = CreateWindowEx(WS_EX_CLIENTEDGE,"WindowClass","Notepad Pro 1.1.4",WS_OVERLAPPEDWINDOW,
		CW_USEDEFAULT,
		CW_USEDEFAULT,
		640,480,
